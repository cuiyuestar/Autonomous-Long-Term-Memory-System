...
[架构](#架构) · [核心能力](#核心能力) · [快速开始](#快速开始) · [Agent 接入](#agent-接入) · [MCP](#mcp-接入) · [评估](#评估)
...
运行时通过两个调用嵌入任意 Host Agent：

```text
prepare_turn -> Host Agent 生成回复 -> commit_turn
```

`prepare_turn` 捕获用户输入并返回 `ContextBundle`。`commit_turn` 保存真实 Assistant 回复、实际引用和反馈。ALTM 不代理 Host Agent 的推理过程。
...
| 接入方式 | Python、CLI、MCP、TypeScript SDK、OpenClaw adapter、Hermes adapter |
...
Python 3.11+ 承担领域逻辑和持久化。TypeScript、OpenClaw 与 Hermes 组件只处理 Host 生命周期映射和协议调用。
