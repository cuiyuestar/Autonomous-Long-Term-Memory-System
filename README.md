...
ALTM 是一个具备记忆自主涌现的agent长期分层记忆系统，基于稀疏/稠密向量混合索引、异构图PPR和多因子相关度计算实现中高层记忆涌现；具备L0-L4分层记忆存储结构，层级间证据链使高层宏观记忆可追溯至低层细粒度记忆；具备完整记忆生命周期管理与升降级机制，记忆权重可随激活频率动态调整；具备基于CCR的上下文治理机制
...
公开评测使用 LongMemEval 和 LoCoMo，对比 L0 原始记忆基线与加入真实远程 Embedding 后的证据检索结果。两组使用相同数据、题目、Top-K 和指标定义；Embedding 组不包含 L1-L4、Graph、Active Window 或 CCR，也不代表最终问答准确率。

| Benchmark | 方案 | Recall-any@1 | Recall-any@5 | Recall-any@10 | Recall-all@10 | nDCG@10 | MRR | p95 延迟 |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| LongMemEval cleaned，前 100 题 | L0 | 47.00% | 73.00% | 80.00% | 62.00% | 58.21% | 58.48% | 468.39 ms |
| LongMemEval cleaned，前 100 题 | L0 + Embedding | 74.00% | 99.00% | 100.00% | 82.00% | 84.54% | 85.82% | 1,032.84 ms |
| LoCoMo 全集，1,982 题可评分 | L0 | 14.68% | 27.75% | 37.13% | 31.69% | 23.01% | 20.68% | 78.95 ms |
| LoCoMo 全集，1,982 题可评分 | L0 + Embedding | 27.40% | 56.66% | 66.40% | 55.05% | 42.96% | 39.53% | 1,248.58 ms |

Embedding 使用 `text-embedding-v4` 的 1,024 维向量。LongMemEval 的 Recall-any@10 提升 20.00 个百分点，LoCoMo 提升 29.26 个百分点；代价是 p95 在线召回延迟分别增至基线的 2.21 倍和 15.82 倍。
...
公开 benchmark 保留各自原始指标，不将不同数据集压缩成一个总分。完整指标定义、基线和实施阶段见[量化评测体系](docs/quantitative-evaluation-framework.md)，L0 基线见[首轮评测报告](reports/initial-eval-20260815/README.md)，Embedding 消融结果见[向量评测报告](reports/embedding-eval-20260816/README.md)。
